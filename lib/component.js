async function findWhereOrUndefined (where, root) {
  ...
  if (root.children) {
    for (let c of root.children) {
      c = await c
      const r = findWhereOrUndefined(where, c)
      if (r !== undefined) {
        return r
      }
    }
...
async function getChildren (root) {
  root = await root
  if (root.children) {
    const all = root.children.map(async e => e)
...
function resolving (component, operator) {
...
async function resolveComponent (component, operators) {
  return operators.reduce(resolving, await component)
}
...
class TestComponent {
  ...
  findWhere (condition) {
    this.operators.push(root => {
      const r = findWhereOrUndefined(condition, root)
      if (r === undefined) {
  ...
  findById (id) {
    this.operators.push(root => {
      const r = findWhereOrUndefined(e => e.props.get('id') === id, root)
      if (r === undefined) {
  ...
  findByClass (className) {
    this.operators.push(root => {
      const r = findWhereOrUndefined(e => e.props.get('class') === className, root)
      if (r === undefined) {
  ...
  findNth (n) {
    this.operators.push(root => {
      const r = findChildrenNth(n, root)
      if (r !== undefined) {
        return r
      }
  ...
  trigger (event) {
    ...
    this.operators.push(root => {
      if (root.props.get(eventName)) {
        return root.props.get(eventName)(event)
      }
